...
import { Typography, Divider, TextField, Box, InputAdornment, IconButton } from '@mui/material';
...
const Account: React.FC = () => {
  const { User, setFirstName, setLastName, setEmail, setPassword, setConfirmPassword, setHasAdminPower, setIsReader } = useStore();
  ...
  const handleSubmitSignup = async (e: MouseEvent<HTMLButtonElement>) => {
    e.preventDefault();
    ...
    try {
      ...
      if (user) {
        ...
        const documentId = User.docId;
        
  
        try {
          // Create custom ID
          await updateDoc(doc(db, 'usersInfo', documentId), { 
            ...
            >
              <Box>
                <Box sx={Boxnav}>
                  ...
                  <Divider />
                  <Box sx={{ display: 'flex', justifyContent: 'space-around', gap: { sm: '25px', md: '35px' }, alignItems: 'center',fontFamily:"DANS SANS", fontSize:"1.2rem" }}>Update your acount</Box>
                </Box>
              </Box>
                ...
                <Box sx={{ width: screenWidth >= 1000 ? '40%' : screenWidth >= 600 ? '80%' : '100%' }}>
                  <Box>
                    <Box sx={{ paddingBlock: '1rem' }}>
                      ...
                      <button onClick={handleSubmitSignup} style={RegBtn}>Update</button>
                      <Box sx={LoginLinkContainer}>
                        <p style={{ marginBottom: screenWidth <= 900 ? "10px" : "18px" }}>Go back to log in? <span onClick={handlelogin} style={{ color: "blue" }}>Log in</span> </p>
